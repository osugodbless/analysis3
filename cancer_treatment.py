...
treatment_groups = ['Surgery Only', 'Surgery + Chemotherapy', 'Chemotherapy Alone (No Surgery)']

# Color scheme: Blue (Surgery), Orange (Both), Red (Chemo Alone)
colors = {
    'Surgery Only': 'steelblue',
    'Surgery + Chemotherapy': 'darkorange',
    'Chemotherapy Alone (No Surgery)': 'darkred'
}

print("\nFitting Aalen-Johansen Models for Treatment Modality...\n")

fitters = {}
robust_cutoffs = {}
max_incidence_decimal = 0

for tx in treatment_groups:
    cohort = df[df['Clean_Treatment'] == tx]
    
    if len(cohort) == 0:
        continue
        
    print(f"--- {tx} (N = {len(cohort):,}) ---")
    ...
    robust_cutoffs[tx] = robust_cutoff
...
for tx in treatment_groups:
    if tx in fitters:
        ajf = fitters[tx]
        cutoff = robust_cutoffs[tx]
        plot_df = ajf.cumulative_density_[ajf.cumulative_density_.index <= cutoff]
        plt.plot(plot_df.index, plot_df.iloc[:, 0], label=tx, color=colors[tx], linewidth=1.5)
...
plt.legend(title="Primary Treatment Modality", loc='upper left')
...
plt.tight_layout()
plt.savefig("Ovarian_Disparities_Treatment_CV_Mortality.png", dpi=300)
print("Graph saved successfully as Ovarian_Disparities_Treatment_CV_Mortality.png!")