...
# Filter out "Unknown/Missing" to keep the comparison strictly geographic
df = df[df['Clean_Rural_Urban'] != 'Unknown/Missing']

# Define the exact order for processing
geography_groups = ['Urban (Metropolitan)', 'Rural (Nonmetropolitan)']

# Distinct colors for high-contrast plotting
colors = {
    'Urban (Metropolitan)': 'steelblue',
    'Rural (Nonmetropolitan)': 'darkorange'
}

print("\nFitting Aalen-Johansen Models for Rural/Urban Geography...\n")

fitters = {}
robust_cutoffs = {}
max_incidence_decimal = 0

for geo in geography_groups:
    cohort = df[df['Clean_Rural_Urban'] == geo]
    
    if len(cohort) == 0:
        continue
        
    print(f"--- {geo} (N = {len(cohort):,}) ---")
    ...
    robust_cutoffs[geo] = robust_cutoff
...
for geo in geography_groups:
    if geo in fitters:
        ajf = fitters[geo]
        cutoff = robust_cutoffs[geo]
        plot_df = ajf.cumulative_density_[ajf.cumulative_density_.index <= cutoff]
        plt.plot(plot_df.index, plot_df.iloc[:, 0], label=geo, color=colors[geo], linewidth=1.5)
...
plt.legend(title="Geographic Location", loc='upper left')
...
plt.tight_layout()
plt.savefig("Ovarian_Disparities_RuralUrban_CV_Mortality.png", dpi=300)
print("Graph saved successfully as Ovarian_Disparities_RuralUrban_CV_Mortality.png!")
plt.show()